...
size_t write_buffer(HANDLE file_descriptor, char *buffer, int len)
{
	...
	while (1) {
		bRet = WriteFile(file_descriptor,
				 buffer + bytes_write,
				 len - bytes_write, &current_write, NULL);

		if ((int)current_write == 0 || bRet == FALSE)
			return -1;
...
size_t so_fwrite(const void *ptr, size_t size, size_t nmemb, SO_FILE *stream)
{
	...
	while (nbytes > 0) {
		...
		if (nbytes > IO_BUFFER_SIZE) {
			rc = write_buffer(stream->file_descriptor, (char *)ptr,
					  IO_BUFFER_SIZE);
			/** in caz de eroare */
			if (rc == -1) {
				stream->error = EIO;
...
int so_fflush(SO_FILE *stream)
{
	...
	if (rc != stream->cursor_buffer_write) {
		stream->error = EIO;
		return SO_EOF;
	}
	stream->cursor_buffer_write = 0;
